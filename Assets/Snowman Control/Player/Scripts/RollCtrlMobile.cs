...
public class RollCtrlMobile : MonoBehaviour {
	...
	// JoyPad
	private float deadZoneH = 10f;
	private float deadZoneV = 10f;
	...
	void FixedUpdate () {
		...
		torqueZ = -JoystickMove.instance.H;
		torqueX = JoystickMove.instance.V;

		if (Mathf.Abs (torqueZ) > deadZoneH) {
			torqueZ = torqueZ > 0f ? torqueZ - deadZoneH : torqueZ + deadZoneH;
		} else {
			torqueZ = 0f;
		}

		if (Mathf.Abs (torqueX) > deadZoneV) {
			torqueX = torqueX > 0f ? torqueX - deadZoneV : torqueX + deadZoneV;
		} else {
			torqueX = 0f;
		}
		...
		// Jump
		if (canJump && JoystickFire.instance.Fire) {
			canJump = false;
